...
describe('Module 4 Project Tests', () => {
  ...
  describe('Spanish Language', () => {
    let textsESP = getEntriesByKeyPrefix(txt.esp,"TEXT")
    for(let [key,value] of textsESP){
     test(`${key} is visible`,()=>{
       render(<App lang="esp" />)
       expect(screen.getByText(value)).toBeVisible()
     })
    ...
    // })
    let labelsESP = getEntriesByKeyPrefix(txt.esp,"LABEL")
    for(let [key,value] of labelsESP){
     test(`${key} is visible`,()=>{
       render(<App lang="esp" />)
       expect(screen.getByLabelText(value)).toBeVisible()
     })
